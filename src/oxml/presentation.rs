...
impl PresentationReader {
    ...
    /// Get presentation slide size
    pub fn slide_size(&self) -> &Option<Size> {
        &self.slide_size
    }
    ...
    fn parse_presentation_xml(&mut self) -> Result<(), PptxError> {
        ...
        // get slide size
        if let Some(rels_xml) = self.package.get_part("ppt/presentation.xml") {
            let xml_str = String::from_utf8_lossy(rels_xml);
            if let Ok(root) = XmlParser::parse_str(&xml_str) {
                let sld_sz = root.find_descendant("sldSz");
                if let Some(sld_sz) = sld_sz {
                    let width = sld_sz
                        .attr("cx")
                    ...
                    if let (Some(width), Some(height)) = (width, height) {
                        self.slide_size = Some(Size::new(width, height));
                    }
                }
            }
        }
